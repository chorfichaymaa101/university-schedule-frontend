...
@Component({
  ...
  styleUrls: ['./time-table-edit.component.css']
})
export class TimeTableEditComponent {
  ...
  activeCell: { row: number, column: string } | null = null;

  getDayFromColumn(column: string, index: number): string {
  ...
  timeTableByYearBySemester(programId: number, year: string): void {
    this.timeTableService.checkIfExists(programId, year).subscribe({
      next: (exist: boolean) => {
        if (!exist) {
          console.log("L'emploi du temps n'existe pas")
        } else {
  ...
  processSessionExams(session: SessionWithDetails, day: string, slot: string): void {
    ...
    forkJoin({
      ...
      next: ({ professor, module, classe }) => {
        ...
        if (!existingDay) {
          existingDay = { day };
          this.dataSource.push(existingDay);
        }
        ...
        // Make sure the table is sorted by day order
        this.dataSource.sort((a, b) =>
          this.dayNames.indexOf(a.day) - this.dayNames.indexOf(b.day)
        );
  ...
  UpdateSessionRow(): void {
    const selectedIndex = Number(this.thirdFormGroup.get('type')?.value); // Convert to number
    ...
    const updatedRow: Session = {
      id: Number(this.thirdFormGroup.get('sessionId')?.value) || 0,
      ...
      groupe: this.thirdFormGroup.get('groupe')?.value?.trim() ?? '', // Fixing groupe field
    };

    console.log('Updated Row:', updatedRow);

    this.sessionService.updateSession(updatedRow).subscribe({
      next: () => {
        console.log('Séance modifiée');

        // Store current form values
        const currentYear = this.firstFormGroup.get('year')?.value;
        const currentProg = this.firstFormGroup.get('prog')?.value;
        const currentId = this.idTimeTable;
  ...
  onCellSelected(cellData: any, column: string): void {
    ...
    if (cellData) {
      ...
      this.thirdFormGroup.patchValue({
        prof: cellData.professorId || null,
        module: cellData.moduleId || null,
        time: cellData.time || null,
        classe: cellData.classId || null,
        day: cellData.day,
        capacity: cellData.capacity.toString() || null,
        groupe: cellData.groupe || null,
        sessionId: cellData.id || null,
        type: typeIndex.toString() || null
      });
      console.log('Form updated with selected cell data:', this.thirdFormGroup.value);
    }
  }
