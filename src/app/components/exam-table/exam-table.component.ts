...
this.thirdFormGroup.get('prog')?.valueChanges.subscribe(programId => {
  if (programId) {
    ...
    this.thirdFormGroup.get('heureDebut')?.valueChanges.subscribe(value => {
      if (value) {
        // Ensure time format doesn't include seconds
        const timeWithoutSeconds = value.slice(0, 5);  // Only keep HH:mm part
        this.thirdFormGroup.get('heureDebut')?.setValue(timeWithoutSeconds, { emitEvent: false });
      }
    });

    this.thirdFormGroup.get('heureFin')?.valueChanges.subscribe(value => {
      if (value) {
        // Ensure time format doesn't include seconds
        const timeWithoutSeconds = value.slice(0, 5);  // Only keep HH:mm part
        this.thirdFormGroup.get('heureFin')?.setValue(timeWithoutSeconds, { emitEvent: false });
      }
    });
